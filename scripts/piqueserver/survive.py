...
def war(connection):
    """
    Enable an alternative spawn setup
    /war
    """
    protocol = connection.protocol
    for bot in protocol.bots:
    ...
    protocol.broadcast_chat("War %s!" % state)
    ...
    class SurviveProtocol(protocol):
        ...
        def update_day_color(self):
            if not self.strong and (self.current_time >= 22.50 or self.current_time <= 4.10):
                self.broadcast_chat("The night begins. Zombies are getting stronger!")
                self.strong = True
