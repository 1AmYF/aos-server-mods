...
from pyspades.contained import InputData, SetTool, WeaponInput, ChatMessage, KillAction
...
def setlevel(connection, level):
    """
    Manually set your level to jump to later parts
    /setlevel <level>
    """
...
def save_stats(connection):
    ...
    f = open(os.path.join(config.config_dir, "adventure_stats.csv"), "a")
        ...
        def on_spawn(self, pos):
            ...
            if self.local:
                ...
                if self.bot_type == 3:
                    self.set_hp(BOT_HP)
            if not self.local:
                if len(self.protocol.bots) < 1:
                    self.protocol.add_adventure_bots()
