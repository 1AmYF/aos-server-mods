...
Setup:
    ...
    To change refill amounts, add this to your server config:
...
from pyspades.server import block_action, weapon_reload
...
def apply_script(protocol, connection, config):
    class BuilderSapperConnection(connection):
        class_mode = None
        block_refills_max = config.get("block_refills", 1)
        block_refills = 0
        ammo_refills_max = config.get("ammo_refills", 1)
        ammo_refills = 0
